...
def train_autogluon(X_train_scaled: pd.DataFrame, y_train: pd.DataFrame) -> TabularPredictor:
    """
    ...
    Returns:
    """
    train_data_df: pd.DataFrame = pd.concat([X_train_scaled, y_train], axis=1).reset_index(drop=True)
